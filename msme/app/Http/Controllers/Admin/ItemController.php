...
class ItemController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $state_id = empty($request->state) ? null : $request->state;
        $items = collect([]);
        ...
        else
        {
            if($state_id)
            {
                ...
                $items_query = $state->items()
                    ->where('country_id', $settings->setting_site_location_country_id);
            }
            else
            {
                $items_query = Item::where('country_id', $settings->setting_site_location_country_id)
                    ->orderBy('updated_at', 'DESC');
            }
        }

        $items_count = $items_query->count();
        $items = $items_query->paginate(10);

        return response()->view('backend.admin.item.index', compact(
            'all_categories', 'all_states', 'category_id', 'state_id', 'items', 'items_count'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return response()->view('backend.admin.item.create',
            compact('all_categories', 'all_states',
                'category_ids', 'all_customFields', 'setting_site_location_lat', 'setting_site_location_lng'));
    }
...
//            'image_gallery.*' => 'image|max:5120',
        ...
        $item_slug = uniqid() . '-' . str_slug($item_title);
        ...
        $default_country = Country::find($settings->setting_site_location_country_id);
    ...
    public function edit(Item $item)
    {
        ...
        return response()->view('backend.admin.item.edit',
            compact('item_owner', 'all_states', 'all_cities', 'all_customFields', 'item', 'categories',
...
//        $custom_field_ids = DB::table('category_custom_field')
//            ->whereIn('category_id', $select_categories)
//            ->distinct('custom_field_id')
//            ->get();
//
//        $select_custom_field = array();
...
//        }
//
//        $category_custom_fields = CustomField::whereIn('id', $select_custom_field)
//            ->orderBy('custom_field_order')
//            ->get();
        ...
        if(is_array($image_gallary) && count($image_gallary) > 0)
        {
            ...
            {
                // only total 12 images are allowed
                if($total_item_image_gallery + $image_gallary_key < 12)
                {
                    $currentDate = Carbon::now()->toDateString();
                    $item_image_gallery_uniqid = uniqid();
