...
@section('content')
    ...
    <div class="row justify-content-between">
        ...
        <div class="col-3 text-right">
            <a href="{{ route('user.products.index') }}" class="btn btn-info btn-icon-split">
        ...
        <div class="col-12">
            ...
            <div class="row mt-4">
                <div class="col-xl-12 col-lg-12 col-sm-12">
                    <form method="POST" action="{{ route('user.products.update', ['product' => $product]) }}" class="">
                        @csrf
                        @method('PUT')
    ...
    @foreach($product_features as $key => $product_feature)
        ...
        <form id="form_feature_rank_up_{{ $product_feature->id }}" action="{{ route('user.product.feature.up', ['product' => $product->id, 'product_feature' => $product_feature->id]) }}" method="POST">
            @csrf
            @method('PUT')
        </form>

        <form id="form_feature_rank_down_{{ $product_feature->id }}" action="{{ route('user.product.feature.down', ['product' => $product->id, 'product_feature' => $product_feature->id]) }}" method="POST">
            @csrf
            @method('PUT')
        </form>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ route('user.product.feature.destroy', ['product' => $product->id, 'product_feature' => $product_feature->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                @if($attributes->count() > 0)
                        ...
                        <div class="modal-body">
                            <form action="{{ route('user.product.attribute.update', ['product' => $product]) }}" method="POST" id="add-product-attribute-form">
                                @csrf
                                @method('PUT')
                ...
                @else
                    <div class="modal-body">
                        {{ __('products.no-attributes-user') }}
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('backend.shared.cancel') }}</button>
                    </div>
