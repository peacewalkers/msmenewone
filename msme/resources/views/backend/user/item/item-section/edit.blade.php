...
@section('content')
    ...
    <div class="row justify-content-between">
        ...
        <div class="col-3 text-right">
            <a href="{{ route('user.items.sections.index', ['item' => $item]) }}" class="btn btn-info btn-icon-split">
    ...
    <div class="row bg-white pt-4 pl-3 pr-3 pb-4">
        <div class="col-12">
            ...
            <div class="row">
                <div class="col-12">
                    <form method="POST" action="{{ route('user.items.sections.update', ['item' => $item, 'item_section' => $item_section]) }}">
                        @csrf
                        @method('PUT')
                    ...
                    <div class="row align-items-center pt-2 mb-2 border-left-info {{ $all_item_section_collections_key%2 == 0 ? 'bg-light' : '' }}">
                        ...
                        @if($collection->item_section_collection_collectible_type == \App\ItemSectionCollection::COLLECTIBLE_TYPE_PRODUCT)
                            ...
                            <div class="col-lg-7 col-md-6 col-sm-12">
                                ...
                                |
                                <span class="text-gray-800">{{ $product_currency_symbol . $find_product->product_price }}</span>
                                |
                                ...
                                @endif
                                |
                                <a target="_blank" class="ml-1" href="{{ route('user.products.edit', ['product' => $collection->item_section_collection_collectible_id]) }}">
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-12">
                                <form class="float-left pr-1" action="{{ route('user.items.sections.collections.rank.up', ['item' => $item, 'item_section' => $item_section, 'item_section_collection' => $collection->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form class="float-left pr-1" action="{{ route('user.items.sections.collections.rank.down', ['item' => $item, 'item_section' => $item_section, 'item_section_collection' => $collection->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-primary btn-sm text-white">
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form action="{{ route('user.items.sections.collections.destroy', ['item' => $item, 'item_section' => $item_section, 'item_section_collection' => $collection->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('backend.shared.cancel') }}</button>
                    <form action="{{ route('user.items.sections.destroy', ['item' => $item, 'item_section' => $item_section->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">{{ __('backend.shared.delete') }}</button>
                    </form>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                @if($available_products->count() > 0)
                    ...
                    <div class="modal-body">
                        <form action="{{ route('user.items.sections.collections.store', ['item' => $item, 'item_section' => $item_section]) }}" method="POST" id="add-product-form">
                            @csrf
                            ...
                            @foreach($available_products as $available_products_key => $available_product)
                                <div class="row align-items-center pt-2 mb-2 border-left-info {{ $available_products_key%2 == 0 ? 'bg-light' : '' }}">
                                    ...
                                    <div class="col-8">
                                        ...
                                        |
                                        <span class="text-gray-800">{{ $product_currency_symbol . $available_product->product_price }}</span>
                                        |
                ...
                @else
                    <div class="modal-body">
                        {{ __('item_section.no-products-user') }}
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('backend.shared.cancel') }}</button>
                    </div>
