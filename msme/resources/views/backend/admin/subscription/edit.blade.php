                    ...
                    <form method="POST" action="{{ route('admin.subscriptions.update', $subscription->id) }}" class="">
                                ...
                                <select class="custom-select" name="plan_id">
                                    ...
                                    @foreach($all_plans as $key => $plan)
                                        <option value="{{ $plan->id }}" {{ (old('plan_id') ? old('plan_id') : $subscription->plan_id) == $plan->id ? 'selected' : '' }}>
                                            {{ $plan->plan_type == \App\Plan::PLAN_TYPE_FREE ? 'Free' : 'Paid' }} |
                                            {{ $plan->plan_name }} |
                        ...
                        <div class="row form-group">
                            <div class="col-md-12">
                                ...
                                <input id="subscription_end_date" type="text" class="form-control @error('subscription_end_date') is-invalid @enderror" name="subscription_end_date" value="{{ old('subscription_end_date') ? old('subscription_end_date') : $subscription->subscription_end_date }}">
